...
public class SineWaveUpSide : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        transform.position = pos + (transform.up * Mathf.Sin(elapsedTime * frequency)*-1 * magnitude); // use time.time instead of elapsedTime
